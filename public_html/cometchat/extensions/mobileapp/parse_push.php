...
class Parse {
	...
	public function sendNotification($channel, $messageData, $isChatroom = '0') {
		...
		if($channel && $messageData) {
			...
			if(strpos($messageData['m'],'cometchat_smiley')!==false){
				preg_match_all('/<img[^>]+\>/i',$messageData['m'],$matches);
				for($i=0;$i<sizeof($matches[0]);$i++){
					$imagename = explode('"',explode('src="/cometchat/cometchat/images/smileys/',$matches[0][$i])[1])[0];
					$messageData['m'] = str_replace($matches[0][$i],$emojiUTF8[$imagename],$messageData['m']);				
				}	
			}
			...
			$notificationData =	array('alert' => $messageData['m'],'t' => $this->messageType,'m' => $messageData,'action' => "PARSE_MSG");
