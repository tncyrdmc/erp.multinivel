...
class modelo_billetera extends CI_Model
{
	...
	function getComisionDirectos($id_afiliado, $id_red)
	{
		...
		where v.id_user = a.id_afiliado and v.id_venta = c.id_venta  and c.id_red = ".$id_red." and  c.id_afiliado = ".$id_afiliado." and  a.debajo_de = ".$id_afiliado); 
	...
	function getComisionDirectosMes($id_afiliado, $id_red, $fecha)
	{
		$q = $this->db->query("select sum(c.puntos) as puntos, sum(c.valor) as valor
		from afiliar a, comision c, venta v
		where v.id_user = a.id_afiliado and v.id_venta = c.id_venta and MONTH(".$fecha.") = MONTH(c.fecha) and c.id_red = ".$id_red." and  c.id_afiliado = ".$id_afiliado." and  a.debajo_de = ".$id_afiliado); 
