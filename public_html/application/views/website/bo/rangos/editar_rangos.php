...
<form id="nueva" class="smart-form"  novalidate="novalidate" >
							<fieldset>
								...
								//if(!isset($RangoVentas)){
								//echo '<label class="input">Tipo Condicion</label>';
							...
							//}
									foreach ($RangoVentas as $rangoventas) {
										...
										echo '';
										echo '<div id="'.$i.'" class="row"><label class="select">Tipo Condicion</label>
										<div class="col col-lg-2">
										</div>';
											echo '<div class="col col-xs-12 col-sm-6 col-lg-3" id="id_tipo_condicion[]">'.'<label class="select">'.'<select id="id_tipo_condicion[]" name="id_tipo_condicion[]" >';
										foreach ($tipo_rango as $categoria) {
										...
										if($i>1){
											echo '<a style="cursor: pointer;" onclick="delete_rango('.$i.')">Eliminar Rango <i class="fa fa-minus"></i></a>';
										}
										$i++;
										echo ''.'</label>'.'</div>'.'</div>';
...
function add_rango(id)
{
	...
	+'<div class="col col-xs-12 col-sm-5 col-lg-3">'
		+'Tarifa<label for="" class="input">'
			...
			+'<a style="cursor: pointer;" onclick="delete_rango1('+i+')">Eliminar Rango <i class="fa fa-minus"></i></a>'
		+'</label>'
	+'</div>'
	+'</div>';
	$("#rango").append(code);
